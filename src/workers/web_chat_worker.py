...
class WebChatWorker(BaseWorker):
    ...
    async def _require_composer(self, page: Page) -> None:
        """
        Wait for the prompt composer, detecting login walls.

        Args:
            page: Provider page.

        Raises:
            WorkerError: If a login wall is shown or the composer never
                appears.
        """
        try:
            await page.locator(self._site.composer_selector).first.wait_for(
                state="visible",
                timeout=self._site.ready_timeout_seconds * 1000,
            )
            return
        except Exception:
            pass

        # Checked before the login wall: a CAPTCHA often accompanies a
        # sign-in prompt, and the challenge is the blocking condition.
        # Raising a distinct type is what lets the session manager pause
        # only this provider instead of retrying it.
        if await self._challenge_visible(page):
            raise ProviderChallengeError(
                f"{self._site.display_name} is showing a human "
                f"verification challenge. Solve it manually in the "
                f"automation Chrome window; this provider is paused and "
                f"research continues without it.",
        ...
        if await self._login_wall_visible(page):
            ...
            self._record_auth(
                AuthState.LOGIN_REQUIRED, "Sign-in screen displayed"
            )
        ...
        raise WorkerError(
            f"{self._site.display_name} composer did not appear. The site "
            f"layout may have changed; check the configured selector "
            f"'{self._site.composer_selector}'.",
            code=f"{self._site.name.upper()}_COMPOSER_NOT_FOUND",
        )
    ...
    async def _submit_prompt(self, page: Page, prompt: str) -> None:
        ...
        if len(prompt) > self._site.paste_threshold_chars:
            await self._keyboard.paste_text(
                page=page,
                text=prompt,
                selector=self._site.composer_selector,
            )
        else:
